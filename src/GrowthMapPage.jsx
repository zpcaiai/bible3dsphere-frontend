...
const DIM_LABELS = {
  ...
  fear_tendency: { zh: t("恐惧倾向"), icon: '⚡', color: '#ff9500' },
  pride_tendency: { zh: t("骄傲倾向"), icon: '🏆', color: '#ff3b30' },
  emotional_stability: { zh: t("情绪稳定"), icon: '💧', color: '#5ac8fa' },
  truth_alignment: { zh: t("真理对齐"), icon: '✝️', color: '#c4b5fd' },
  relational_health: { zh: t("关系健康"), icon: '❤️', color: '#ff6b9d' },
  resilience: { zh: t("韧性"), icon: '🌊', color: '#00d2ff' },
  spiritual_clarity: { zh: t("属灵清晰度"), icon: '🌟', color: '#ffd700' },
}
...
export default function GrowthMapPage({ user, token, onBack }) {
  ...
  const ARC_LABELS = { breaking_through: t("🌟 突破成长中"), deepening_loops: t("🔄 循环加深"), stabilizing: t("🌱 趋于稳定"), unknown: t("🔮 轨迹未知") }
  const TRAJ_LABELS = { stabilizing: t("🌱 稳定成长"), improving_clarity: t("✨ 属灵清晰度提升"), fragmenting: t("🌊 内心正在挣扎"), increasing_volatility: t("⚡ 情绪波动较大"), cyclical: t("🔄 循环模式中") }
  const LOOP_LABELS = { fear_control_loop: t("🔒 恐惧控制循环"), shame_avoidance_loop: t("🙈 羞耻回避循环"), pride_comparison_loop: t("🏆 骄傲比较循环"), desire_impulse_loop: t("🌊 欲望冲动循环"), truth_stability_loop: t("✨ 真理稳固循环") }
    ...
    <div className="pw-page">
      <header className="pw-header">
        ...
        <div className="pw-header-center">
          <div className="pw-title">{t("📊 灵命成长图谱")}</div>
          <div className="pw-subtitle">{t("SFDS 8维度灵命轨迹可视化")}</div>
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {[['chart', t("图谱")], ['milestones', t("徽章")]].map(([k, l]) => (
        ...
        {loading ? (
            ...
            {Object.keys(stateVector).length > 0 ? (
              <>
                ...
                <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 14, padding: '18px' }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.6)', marginBottom: 14 }}>{t("8维度详细数值")}</div>
              ...
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'rgba(255,255,255,0.4)', lineHeight: 1.7 }}>
                ...
                <div>{t("完成几次情绪打卡或心迹分析后")}</div>
                <div>{t("图谱将自动生成你的灵命轨迹")}</div>
              </div>
            )}
          </>
        ) : (
          ...
          <>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 20 }}>
              {t("里程碑是你属灵旅程中的见证，不是成就，而是回忆。")}
            </div>
            {milestones.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'rgba(255,255,255,0.3)', fontSize: 14, lineHeight: 1.8 }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>🌱</div>
                <div>{t("旅程才刚刚开始")}</div>
                <div style={{ fontSize: 12, marginTop: 8 }}>{t("连续7天灵修、提交10条代祷、回答7次灵魂一问...")}</div>
                <div style={{ fontSize: 12 }}>{t("每一步都会被记录。")}</div>
              </div>
            ) : (
            ...
            <div style={{ marginTop: 24 }}>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 12 }}>{t("🔒 待解锁")}</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {[
                  { icon: '🌿', name: t("旷野七日"), desc: t("连续7天灵修") },
                  { icon: '🕯️', name: t("月光守望"), desc: t("连续30天灵修") },
                  { icon: '🙏', name: t("守望者"), desc: t("提交10条代祷") },
                  { icon: '✝️', name: t("信心见证者"), desc: t("3个祷告蒙恩答应") },
                  { icon: '🔍', name: t("七日自省者"), desc: t("回答7次灵魂一问") },
                  { icon: '📖', name: t("书卷完成者"), desc: t("读完整卷圣经") },
