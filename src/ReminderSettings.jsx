...
export default function ReminderSettings({ onBack }) {
  ...
  async function enable() {
    ...
    try {
      ...
      if (perm !== 'granted') { setMsg(t("需要允许通知权限才能开启提醒")); setBusy(false); return }
      ...
      setSubscribed(true); setMsg(t("✓ 提醒已开启"))
    } catch (e) { setMsg(t("开启失败：") + (e.message || '')) }
  ...
  async function saveTimes() {
    ...
    try {
      ...
      setMsg(t("✓ 已保存"))
    } catch (e) { setMsg(e.message || t("保存失败")) }
    finally { setBusy(false) }
  }
  ...
  async function sendTest() {
    ...
    setMsg(r.sent ? `✓ 已发送 ${r.sent} 条测试推送` : (r.reason || t("发送失败（请先开启提醒）")))
    setBusy(false)
  }
  ...
  return (
    <div style={{ width: '100%', height: '100%', background: '#000', color: '#fff', overflowY: 'auto', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div style={{ padding: '14px 16px 100px', maxWidth: 600, margin: '0 auto' }}>
        {!supported && <div style={{ ...card, color: '#ffd43b' }}>{t("当前浏览器不支持 Web 推送。建议把应用「添加到主屏」后再试，或换用 Chrome / Edge。")}</div>}
        {configured === false && supported && (
          <div style={{ ...card, color: '#ffd43b', fontSize: 13, lineHeight: 1.7 }}>
            {t("服务器尚未配置推送密钥（VAPID）。配置后即可开启。你仍可在下方设置提醒时间，开启后生效。")}
          </div>
        )}
        ...
        <div style={{ ...card, background: 'linear-gradient(135deg, rgba(90,200,250,0.10), rgba(52,199,89,0.08))' }}>
          <div style={{ fontSize: 13, lineHeight: 1.75, color: 'rgba(255,255,255,0.8)' }}>
            {t("习惯不是靠意志，而是靠节奏。开启后，你会在选定的时间收到一句温柔的提醒——\n            晨更迎接新的怜悯，晚祷与神同回顾这一天。")}
          </div>
        </div>

        <div style={card}>
          <Row label={t("🌅 晨更提醒")} on={morningOn} setOn={setMorningOn} time={morningTime} setTime={setMorningTime} />
          <div style={{ height: 1, background: 'rgba(255,255,255,0.06)', margin: '14px 0' }} />
          <Row label={t("🌙 晚祷提醒")} on={eveningOn} setOn={setEveningOn} time={eveningTime} setTime={setEveningTime} />
        </div>

        {!subscribed ? (
          <button onClick={enable} disabled={busy || !supported} style={primaryBtn}>{busy ? t("处理中…") : t("开启提醒")}</button>
        ) : (
          <>
            <button onClick={saveTimes} disabled={busy} style={primaryBtn}>{busy ? t("保存中…") : t("保存提醒时间")}</button>
            <button onClick={sendTest} disabled={busy} style={{ ...primaryBtn, background: 'rgba(255,255,255,0.08)', marginTop: 8 }}>{t("发送一条测试推送")}</button>
          </>
        )}
