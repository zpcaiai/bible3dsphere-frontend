...
const STEPS = [
  ...
  { key: 'feeling', q: t("你感受到什么？"), ph: t("焦虑、愤怒、羞耻、悲伤、空虚…") },
  { key: 'want',    q: t("你真正想要的是什么？"), ph: t("在这感受底下，你渴望得到 / 留住什么？") },
  { key: 'fear',    q: t("你最害怕失去什么？"), ph: t("如果它没了，你会崩溃的是什么？") },
  { key: 'belief',  q: t("这让你相信了关于神 / 自己的什么？"), ph: t("诚实地写，哪怕是「神不管我」…") },
]
...
export default function GospelDiagnosticPage({ user, onBack, onNeedLogin }) {
  ...
  async function submit() {
    ...
    catch (e) { setError(e.message || t("诊断失败")) } finally { setBusy(false) }
  }
    ...
    <div style={{ width: '100%', height: '100%', background: '#000', color: '#fff', overflowY: 'auto', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(28,28,30,0.92)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(10px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <BackButton onClick={view !== 'form' ? () => setView('form') : onBack} />
        ...
        <button onClick={view === 'history' ? () => setView('form') : openHistory} style={pill}>{view === 'history' ? t("← 返回") : t("病历")}</button>
      </div>
      ...
      <div style={{ padding: '14px 16px 100px', maxWidth: 680, margin: '0 auto' }}>
          ...
          <>
            <div style={{ ...card, background: 'linear-gradient(135deg, rgba(139,92,246,0.12), rgba(90,200,250,0.06))' }}>
              <div style={{ fontSize: 13, lineHeight: 1.75, color: 'rgba(255,255,255,0.82)' }}>
                {t("症状不是问题。让我们一层层往下看——情绪揭示信念，信念揭示偶像，偶像揭示不信；\n                而福音，正是对付不信的良药。")}
              </div>
            </div>
            <div style={card}>
              <div style={{ fontSize: 11, color: '#a78bfa', fontWeight: 700, marginBottom: 8 }}>{t("第")} {step + 1} / {STEPS.length} {t("问")}</div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
                {step > 0 && <button onClick={() => setStep(step - 1)} style={{ ...btn('rgba(255,255,255,0.08)'), color: 'rgba(255,255,255,0.7)', flex: 1 }}>{t("上一步")}</button>}
                {step < STEPS.length - 1
                  ? <button onClick={() => setStep(step + 1)} style={{ ...btn('linear-gradient(135deg,#8b5cf6,#5ac8fa)'), flex: 2 }}>{t("下一步")}</button>
                  : <button onClick={submit} disabled={busy} style={{ ...btn('linear-gradient(135deg,#8b5cf6,#5ac8fa)'), flex: 2 }}>{busy ? t("诊断中…") : t("生成属灵病历")}</button>}
              </div>
            </div>
        ...
        {view === 'result' && result && (
          <>
            ...
            <div style={{ ...card, borderColor: 'rgba(218,119,242,0.3)' }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#da77f2', marginBottom: 12 }}>{t("🔬 钟马田 · 诊断（挖到根）")}</div>
              {[[t("情绪"), result.emotion], [t("渴望"), result.desire], [t("害怕失去"), result.fear_named], [t("偶像"), result.idol_name], [t("底层的不信"), result.unbelief]].map(([k, v], i) => (
        ...
        {view === 'history' && (
          history.length === 0
            ? <div style={{ ...card, textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}>{t("还没有诊断记录")}</div>
            : history.map((h, i) => (
