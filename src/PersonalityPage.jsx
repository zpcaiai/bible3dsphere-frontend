...
const REFLECTION_CATEGORIES = [
  {
    ...
    label: t("第一类：与神的关系（核心根基）"),
    emoji: '🙏',
    color: '#fbbf24',
    lesson: t("信靠、亲近神、听从圣灵"),
    questions: [
      t("我每天/每周与神亲密相交（读经+祷告）的时间和品质如何？是否流于形式？"),
      t("当我遇到困难时，第一反应是倚靠神、还是先靠自己或他人？"),
      t("我最近在读经或听道时，神最常感动或责备我的经文或主题是什么？"),
      t("我是否真正相信神在我生命中掌权，并为我有美好的计划？（有没有隐藏的不信或埋怨？）"),
    ]
  },
  {
    key: 'character',
    label: t("第二类：品格与内心（圣灵果子）"),
    emoji: '🌿',
    color: '#4ade80',
    lesson: t("忍耐、谦卑、饶恕、节制"),
    questions: [
      t("在压力、委屈或被批评时，我最常表现出什么情绪或行为？（如愤怒、退缩、控诉）"),
      t("我生命中缺少圣灵果子（加拉太书5:22-23）最明显的是哪一项？（仁爱、喜乐、和平、忍耐、恩慈、良善、信实、温柔、节制）"),
      t("我是否容易嫉妒、比较、或在意他人对我的看法？"),
      t("我在小事上是否诚实、守时、尽责？有哪些「小罪」我常常轻忽？"),
    ]
  },
  {
    key: 'relationships',
    ...
    emoji: '🤝',
    color: '#f472b6',
    lesson: t("爱人如己、饶恕、谦卑服事"),
    questions: [
      t("我与最亲近的人（配偶、家人、朋友）最近最常发生的冲突是什么？背后的原因是？"),
      t("我是否主动关心他人、饶恕他人，还是容易记恨或论断？"),
      t("在教会或小组中，我是积极建造他人，还是比较被动或只索取？"),
      t("我是否害怕被拒绝，而不敢真实敞开自己的软弱？"),
    ]
  },
  {
    key: 'trials',
    label: t("第四类：试炼与试探（神常用的「教室」）"),
    emoji: '🔥',
    color: '#f87171',
    lesson: t("顺服、放下偶像、在患难中喜乐"),
    questions: [
      t("过去半年到一年，我最常重复遇到的试炼或挫折是什么？"),
      t("在这些试炼中，我最常问神「为什么」，还是问「你要教导我什么」？"),
      t("我有哪些反复的试探或老我习惯（例如懒惰、贪婪、色欲、忧虑）？"),
      t("如果神现在要我「放下」某样东西（人、事、物、习惯），我最舍不得的是什么？"),
    ]
  },
  {
    key: 'calling',
    label: t("第五类：事奉、呼召与果子"),
    emoji: '🎯',
    color: '#60a5fa',
    lesson: t("忠心、传福音、结果子"),
    questions: [
      t("我如何使用神给我的时间、金钱、恩赐？是否以神国为优先？"),
      t("我在职场、家庭或教会中的见证，是否让人看见基督的不同？"),
      t("我是否清楚自己目前的呼召？有没有在逃避或拖延？"),
      t("身边的人（包括未信者）因为我的生命而更靠近神了吗？"),
    ]
  }
]

const FREQUENCY_OPTIONS = [
  { value: 9, label: t("🌳稳定操练"), color: '#4ade80', desc: t("稳定活出") },
  { value: 5, label: t("🌿成长中"), color: '#fbbf24', desc: t("有所意识") },
  { value: 2, label: t("🌱盲点"), color: '#f87171', desc: t("尚需成长") },
]
...
export default function PersonalityPage({ user, embedded = false, onSyncToHabits }) {
  ...
  const getDominantLoop = () => {
    return profile?.profile?.dominant_loop || t("暂无数据")
  }
  ...
  const dimensionNames = {
    humility: t("谦逊"),
    fear_tendency: t("恐惧倾向"),
    pride_tendency: t("骄傲倾向"),
    emotional_stability: t("情绪稳定"),
    truth_alignment: t("真理对齐"),
    relational_health: t("关系健康"),
    resilience: t("韧性"),
    spiritual_clarity: t("灵性清晰")
  }

  // 弧线描述
  const arcDescriptions = {
    breaking_through: { emoji: '🌅', text: t("突破期"), desc: t("健康的维度正在增强") },
    deepening_loops: { emoji: '🔄', text: t("循环深化"), desc: t("需要注意的行为模式") },
    stabilizing: { emoji: '⚖️', text: t("稳定期"), desc: t("整体趋于平衡") },
    unknown: { emoji: '❓', text: t("未知"), desc: t("数据不足") }
  }

  // 轨迹方向描述
  const trajectoryDescriptions = {
    stabilizing: { emoji: '📈', text: t("趋于稳定"), color: '#4ade80' },
    fragmenting: { emoji: '⚠️', text: t("趋于分散"), color: '#f87171' },
    improving_clarity: { emoji: '✨', text: t("清晰度提升"), color: '#60a5fa' },
    increasing_volatility: { emoji: '📉', text: t("波动性增加"), color: '#fb923c' },
    cyclical: { emoji: '🔄', text: t("周期性"), color: '#fbbf24' },
    unknown: { emoji: '❓', text: t("未知"), color: '#9ca3af' }
  }
  ...
  const computeFormationAnalysis = () => {
    ...
    const maturityStage = overallAvg >= 8 ? { label: t("成熟稳固期"), color: '#4ade80', emoji: '🌳', desc: t("生命根基稳固，持续结出果子") }
      : overallAvg >= 6 ? { label: t("成长深化期"), color: '#60a5fa', emoji: '🌿', desc: t("正在深化，需持续操练") }
      : overallAvg >= 4 ? { label: t("挣扎突破期"), color: '#fbbf24', emoji: '🌱', desc: t("已有意识，仍需神的工作") }
      : { label: t("起步觉醒期"), color: '#f87171', emoji: '🌰', desc: t("正在被唤醒，神要开始塑造工作") }
    ...
    const planMap = {
      god_relationship: {
        short: [t("每天固定时间读经30分钟，用SOAP方法（观察、应用、祷告）"), t("每周写一篇与神相遇的日记"), t("用诗篇139:23-24开始每天的祷告")],
        mid: [t("完成一套系统读经计划（如一年读完圣经）"), t("学习默观祷告或安静等候神"), t("寻找属灵导师，每月分享灵性状况")]
      },
      character: {
        short: [t("每天睡前记录一件\"圣灵果子\"表现和一件失败的事"), t("当感到愤怒时，用10秒暂停法并默祷腓4:7"), t("选一项最弱的圣灵果子专项操练（如节制）")],
        mid: [t("研读登山宝训（马太福音5-7章），每章写应用"), t("加入读书小组共同研读品格塑造资料"), t("邀请3位亲密朋友给你品格上的诚实反馈")]
      },
      relationships: {
        short: [t("本周主动联系一位许久未联系的人，不带目的关心"), t("练习\"先聆听后说话\"：本周开口前先问一个问题"), t("写下需要饶恕的人，用祷告交托")],
        mid: [t("系统学习非暴力沟通（NVC）或圣经中的和好原则"), t("与配偶/家人建立每周固定的深度对话时间"), t("在小组中担任服事职责，操练委身与舍己")]
      },
      trials: {
        short: [t("每天感恩日记：写3件患难中仍感恩的事"), t("遇到试炼时，先问\"神要教我什么\"再求解脱"), t("找出一个反复出现的老我习惯，每周记录一次进展")],
        mid: [t("研读约伯记或罗马书8章，记录对苦难的理解变化"), t("识别生命中的\"偶像\"：将神以外的依靠列出来一一交托"), t("与辅导者共同探索试炼背后的根源模式")]
      },
      calling: {
        short: [t("写下自己认为神给的3个恩赐，并找本周用上的机会"), t("向一位未信友分享近期的生命见证"), t("检视时间/金钱分配：记录本周用在神国上的比例")],
        mid: [t("完成恩赐评估测试并与小组讨论"), t("制定\"事奉地图\"：在家庭、教会、职场各一项具体服事"), t("设立传福音目标：为身边3位未信者持续代祷并寻求机会")]
      }
    }
    ...
    // 基于8维分数生成"今日可行一步"
    const DIM_ACTIONS = {
      humility:            { name:t("谦逊"),    icon:'🌿', threshold:0.5, action:t("今天遇到被否定的时刻，练习在心里先说：「我不需要被证明是对的。」") },
      emotional_stability: { name:t("情绪稳定"), icon:'🌊', threshold:0.5, action:t("设置一个\"情绪暂停\"提醒——每天下午3点用3分钟做一次自我觉察：此刻我的内心在哪里？") },
      truth_alignment:     { name:t("真理对齐"), icon:'📖', threshold:0.5, action:t("今天读一段经文，把它写在手机备忘录里，下班前回顾一次它如何影响了今天的决定。") },
      relational_health:   { name:t("关系健康"), icon:'❤️',  threshold:0.5, action:t("今天主动给一位关系有些疏远的人发一条简短消息，不带目的，只是关心。") },
      resilience:          { name:t("韧性"),    icon:'🌳', threshold:0.5, action:t("找出本月一件让你感到气馁的事，写一句话：「神在这件事上想教我……」") },
      spiritual_clarity:   { name:t("灵性清晰"), icon:'✨', threshold:0.5, action:t("今天安静10分钟，不读书不听音频，只问神：「祢今天最想对我说什么？」然后写下来。") },
      fear_tendency:       { name:t("恐惧倾向"), icon:'😨', threshold:0.4, action:t("识别今天让你焦虑的一件具体事，写下：「最坏的结果是什么？神在最坏的结果里仍然是谁？」"), inverse:true },
      pride_tendency:      { name:t("骄傲倾向"), icon:'🦅', threshold:0.4, action:t("今天找一个机会主动赞美他人的优点，不加任何\"但是\"，看自己内心的感受。"), inverse:true },
    }
  ...
  const FormationInsightPanel = () => {
    if (!formationAnalysis) return (
      <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '14px', padding: '20px', marginBottom: '24px', textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: '14px' }}>
        {t("📋 请先在「反思问题」标签完成问卷，此处将自动生成生命成熟度分析与灵修计划")}
      </div>
    )
      ...
      <div style={{ marginBottom: '28px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(139,92,246,0.15) 0%, rgba(59,130,246,0.12) 100%)', borderRadius: '16px', padding: '22px', marginBottom: '16px', border: '1px solid rgba(139,92,246,0.3)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', marginBottom: '2px' }}>{t("生命成熟度阶段")}</div>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginTop: '4px' }}>{t("综合得分")}</div>
            </div>
          </div>
          {/* 成熟度条 */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
          <div style={{ background: 'rgba(248,113,113,0.08)', borderRadius: '14px', padding: '18px', border: '1px solid rgba(248,113,113,0.25)' }}>
            <div style={{ fontSize: '13px', color: '#f87171', fontWeight: 700, marginBottom: '10px' }}>{t("🎯 当前生命功课")}</div>
            {topLessons.map(c => (
          ...
          <div style={{ background: 'rgba(74,222,128,0.08)', borderRadius: '14px', padding: '18px', border: '1px solid rgba(74,222,128,0.25)' }}>
            <div style={{ fontSize: '13px', color: '#4ade80', fontWeight: 700, marginBottom: '10px' }}>{t("💪 生命强项")}</div>
            {strongCats.map(c => (
              <div key={c.key} style={{ marginBottom: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                  <span style={{ fontSize: '14px' }}>{c.emoji}</span>
                ...
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', paddingLeft: '20px' }}>{c.avg.toFixed(1)} {t("/ 10分")}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        {dimActions && dimActions.length > 0 && (
          <div style={{ background: 'rgba(52,199,89,0.06)', borderRadius: '14px', padding: '18px 20px', marginBottom: '16px', border: '1px solid rgba(52,199,89,0.18)' }}>
            <div style={{ fontSize: '13px', color: '#34c759', fontWeight: 700, marginBottom: '12px' }}>{t("🌿 今日可行一步 — 成长软肋行动")}</div>
        ...
        <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '14px', padding: '20px', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ fontSize: '14px', color: '#c4b5fd', fontWeight: 700, marginBottom: '16px' }}>{t("📅 个人灵修操练计划")}</div>
          <div style={{ display: 'grid', gap: '16px' }}>
            {/* 短期 */}
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                <span style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '10px', background: 'rgba(96,165,250,0.2)', color: '#60a5fa', fontWeight: 700 }}>{t("短期 · 一个月")}</span>
              </div>
              <div style={{ display: 'grid', gap: '8px' }}>
            ...
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                <span style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '10px', background: 'rgba(167,139,250,0.2)', color: '#a78bfa', fontWeight: 700 }}>{t("中期 · 3–6个月")}</span>
              </div>
              <div style={{ display: 'grid', gap: '8px' }}>
                {midPlan.map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
          ...
          <div style={{ marginTop: '16px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            ...
            >
              {syncStatus === 'syncing' ? t("⏳ 同步中...") : t("🌱 同步短期计划到习惯")}
            </button>
            <button
            ...
            >
              {syncStatus === 'syncing' ? t("⏳ 同步中...") : t("📅 同步中期计划到习惯")}
            </button>
          </div>
          ...
          <div style={{ marginTop: '12px', fontSize: '11px', color: 'rgba(255,255,255,0.3)', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '12px' }}>
            {t("💡 计划基于你的反思问卷得分自动生成。每3–6个月重做问卷，计划将随生命成长自动更新。")}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ 
            ...
            return (
              <div style={{
                ...
                <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '13px', lineHeight: 1.7 }}>
                  {t("形成弧线、轨迹方向、主导循环等指标需要通过")}<strong style={{ color: 'rgba(255,255,255,0.75)' }}>{t("情感打卡")}</strong>{t("或")}<strong style={{ color: 'rgba(255,255,255,0.75)' }}>{t("决策分析")}</strong>{t("积累数据后自动生成。")}<br/>
                  {t("每次打卡都会写入一条成长记录，积累足够数据后此处将显示你的性格轨迹。")}
                </div>
              </div>
            )
          }
          return (
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '4px' }}>{t("形成弧线")}</div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '4px' }}>{t("轨迹方向")}</div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '4px' }}>{t("主导循环")}</div>
                <div style={{ fontSize: '14px', fontWeight: 600, color: '#fff' }}>
                  {getDominantLoop() === 'none' ? t("积累中...") : getDominantLoop()}
                </div>
              </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '32px', marginBottom: '8px' }}>📊</div>
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '4px' }}>{t("数据点数")}</div>
      ...
      <div style={{ 
        ...
        {[
          { key: 'reflection', label: t("反思问题"), emoji: '�' },
          { key: 'dimensions', label: t("维度分析"), emoji: '🎯' },
          { key: 'loops', label: t("循环模式"), emoji: '🔄' },
          { key: 'overview', label: t("总览"), emoji: '�' }
        ].map(tab => (
          <button
            key={tab.key}
        ...
        <div>
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', color: 'rgba(255,255,255,0.6)', fontSize: '14px' }}>
              {t("这些数值表示行为倾向（0.05-0.95），不是道德评分。0.5是基线，偏离表示倾向性。")}
            </p>
      ...
      {activeTab === 'dimensions' && (
        <div>
          ...
          <div style={{
            ...
            <strong style={{ color: '#818cf8' }}>{t("📌 维度解读说明：")}</strong> {t("所有数值为行为倾向（0.05–0.95），")}<strong>{t("0.5 是基线")}</strong>{t("。健康维度越高越好；循环倾向维度越低越健康。变化方向（↗↘）反映近期趋势，不是终身标签。")}
          </div>
          ...
          <div style={{ marginBottom: '32px' }}>
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', color: '#4ade80', fontWeight: 600 }}>{t("健康成长维度")}</h3>
              <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginLeft: '4px' }}>{t("越高越好")}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
              {['humility', 'emotional_stability', 'truth_alignment', 'relational_health', 'resilience', 'spiritual_clarity'].map(key => {
                ...
                const statusLabel = isHigh ? { text: t("强"), color: '#4ade80' } : isLow ? { text: t("待培育"), color: '#f87171' } : { text: t("基线"), color: '#fbbf24' }
                return (
                  <div key={key} style={{
          ...
          <div style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
              ...
              <h3 style={{ margin: 0, fontSize: '16px', color: '#f87171', fontWeight: 600 }}>{t("循环倾向维度")}</h3>
              <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginLeft: '4px' }}>{t("越低越健康")}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
              {['fear_tendency', 'pride_tendency'].map(key => {
                ...
                const isHigh = score >= 0.65
                const isLow = score <= 0.35
                const statusLabel = isHigh ? { text: t("活跃"), color: '#f87171' } : isLow ? { text: t("受控"), color: '#4ade80' } : { text: t("基线"), color: '#fbbf24' }
                return (
                  <div key={key} style={{
                    background: 'rgba(0,0,0,0.25)',
                    borderRadius: '14px',
          ...
          <div style={{
            ...
            <h4 style={{ margin: '0 0 12px 0', color: '#c4b5fd', fontSize: '15px' }}>{t("� 维度对比分析")}</h4>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '12px' }}>
              {[
                { label: t("最强维度"), value: (() => { const entries = Object.entries(dimensionNames); const best = entries.filter(([k]) => !['fear_tendency','pride_tendency'].includes(k)).sort(([a],[b]) => getDimensionScore(b) - getDimensionScore(a))[0]; return best ? `${best[1]} (${(getDimensionScore(best[0])*100).toFixed(2)}%)` : '—' })(), color: '#4ade80' },
                { label: t("最需培育"), value: (() => { const entries = Object.entries(dimensionNames); const worst = entries.filter(([k]) => !['fear_tendency','pride_tendency'].includes(k)).sort(([a],[b]) => getDimensionScore(a) - getDimensionScore(b))[0]; return worst ? `${worst[1]} (${(getDimensionScore(worst[0])*100).toFixed(2)}%)` : '—' })(), color: '#f87171' },
                { label: t("最活跃循环倾向"), value: (() => { const fear = getDimensionScore('fear_tendency'); const pride = getDimensionScore('pride_tendency'); return fear > pride ? `恐惧倾向 (${(fear*100).toFixed(2)}%)` : `骄傲倾向 (${(pride*100).toFixed(2)}%)` })(), color: '#f87171' },
                { label: t("健康维度平均"), value: (() => { const healthy = ['humility','emotional_stability','truth_alignment','relational_health','resilience','spiritual_clarity']; const avg = healthy.reduce((s, k) => s + getDimensionScore(k), 0) / healthy.length; return `${(avg*100).toFixed(2)}%` })(), color: '#60a5fa' },
              ].map(item => (
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
              <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '16px' }}>
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>{t("形成弧线")}</div>
              ...
              <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '16px' }}>
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>{t("轨迹方向")}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '12px', padding: '16px' }}>
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>{t("主导循环")}</div>
                <div style={{ fontSize: '16px', fontWeight: 600, color: getDominantLoop() === 'none' ? 'rgba(255,255,255,0.3)' : '#f6ad55' }}>
                  {getDominantLoop() === 'none' ? t("数据积累中...") : getDominantLoop().replace(/_/g, ' ')}
                </div>
              </div>
            </div>
          </div>
              ...
              {[
                {
                  ...
                  name: t("恐惧控制循环"),
                  chain: [t("恐惧"), t("控制"), t("过度工作"), t("燃尽"), t("恐惧")],
                  triggers: [t("未知结果"), t("失去掌控感"), t("完美主义")],
                  response: t("觉察恐惧源头，练习交托祷告，设定边界"),
                  color: '#f87171'
                },
                {
                  key: 'shame_avoidance_loop',
                  name: t("羞耻回避循环"),
                  chain: [t("羞耻"), t("回避"), t("拖延"), t("焦虑"), t("羞耻")],
                  triggers: [t("失败经历"), t("被评价"), t("暴露脆弱")],
                  response: t("在基督里领受完全接纳，小步面对而非逃避"),
                  color: '#fb923c'
                },
                {
                  key: 'pride_comparison_loop',
                  name: t("骄傲比较循环"),
                  chain: [t("骄傲"), t("比较"), t("焦虑"), t("不稳定"), t("骄傲")],
                  triggers: [t("他人成功"), t("身份受威胁"), t("优越感受损")],
                  response: t("默想十架谦卑，专注神眼中的价值"),
                  color: '#fbbf24'
                },
                {
                  key: 'desire_impulse_loop',
                  name: t("欲望冲动循环"),
                  chain: [t("欲望"), t("冲动行为"), t("后悔"), t("欲望")],
                  triggers: [t("即时满足诱惑"), t("逃避痛苦"), t("习惯性渴求")],
                  response: t("暂停10秒，默想永恒奖赏，寻求圣灵大能"),
                  color: '#a78bfa'
                },
                {
                  key: 'truth_stability_loop',
                  name: t("真理稳定循环"),
                  chain: [t("面对真理"), t("反思"), t("稳定"), t("成长"), t("面对真理")],
                  triggers: [t("诚实面对自己"), t("接纳光照"), t("悔改更新")],
                  response: t("保持透明，持续省察，在恩典中成长"),
                  color: '#4ade80',
                  healthy: true
                }
              ].map(loop => (
                ...
                >
                  <div style={{
                    ...
                    <span style={{
                      ...
                      {getDominantLoop() === loop.key ? t("● 当前主导") : loop.healthy ? t("✓ 健康") : t("循环")}
                    </span>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
                    <div>
                      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginBottom: '4px' }}>{t("常见触发")}</div>
                    ...
                    <div>
                      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginBottom: '4px' }}>{t("应对方向")}</div>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', padding: '24px', color: 'rgba(255,255,255,0.4)', fontSize: '13px' }}>
              ...
              <div>{t("循环觉察功能即将上线")}</div>
              <div style={{ fontSize: '11px', marginTop: '4px' }}>{t("记录你观察到的循环模式，追踪突破进度")}</div>
            </div>
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 16px 0', color: 'rgba(255,255,255,0.85)', fontSize: '14px', lineHeight: 1.8 }}>
              {t("神在你生命中特别要塑造你成为耶稣基督那样慈爱怜悯、柔和谦卑、舍己爱人、俯就卑微的罪人，完全顺服天父的旨意这样的品格。要对付的根源问题，或要学习的功课（如信靠、饶恕、顺服、谦卑等）。透过一套有结构的自省题目，你可以系统地找出重复模式、试炼焦点与盲点，帮助你明白神目前的「功课」是什么。")}
            </p>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '13px', color: '#a78bfa', fontWeight: 600, marginBottom: '10px' }}>{t("🙏 开始前")}</div>
              <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7 }}>
                {t("用祷告开始（诗篇139:23-24），求圣灵光照。")}
              </div>
              <div style={{ marginTop: '12px', fontSize: '13px', color: '#a78bfa', fontWeight: 600, marginBottom: '10px' }}>{t("📝 作答原则")}</div>
              <div style={{ display: 'grid', gap: '6px' }}>
                {[
                  t("诚实回答，不急着写「正确答案」。"),
                  t("写下具体例子。"),
                  t("完成后找出共同主题（例如多次提到「愤怒」或「不信」），那很可能就是当前生命功课。"),
                  t("每3–6个月重做一次，观察变化。")
                ].map((tip, i) => (
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                ...
                <span style={{ color: '#fbbf24', fontWeight: 700, fontSize: '13px' }}>{t("重要提醒")}</span>
              </div>
              <div style={{ display: 'grid', gap: '8px' }}>
                {[
                  { ref: t("罗马书12:2"), text: t("这不是为了自我定罪，而是邀请神来更新。") },
                  { ref: t("希伯来书12:5-11"), text: t("生命功课常在重复的痛苦或试炼中显露，神是用爱来修剪。") },
                  { ref: t("加拉太书5:16"), text: t("靠恩典而行：认清功课后，立刻认罪、接受赦免，并倚靠圣灵改变。") },
                ].map((item, i) => (
            ...
            <div style={{
              ...
              <div style={{ fontSize: '13px', color: '#60a5fa', fontWeight: 600, marginBottom: '10px' }}>{t("🔍 分析与应用步骤（逻辑归纳）")}</div>
              <div style={{ display: 'grid', gap: '8px' }}>
                {[
                  { n: '1', text: t("找出模式：哪些题目你的答案最负面或最常出现同一主题？") },
                  { n: '2', text: t("连结圣经：针对该主题查考相关经文（例如若是不饶恕，就看马太福音6:14-15）。") },
                  { n: '3', text: t("写下当前生命功课：例如「神要我学习在工作中信靠祂的供应，而不是忧虑」。") },
                  { n: '4', text: t("制定行动：用生命之轮或之前的地图，设计具体操练（例如每日为该功课用ACTS祷告）。") },
                  { n: '5', text: t("找人确认：与成熟基督徒或小组分享，让他们帮助你看见可能忽略的盲点。") },
                ].map(step => (
          ...
          <div style={{
            ...
            <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.45)', alignSelf: 'center' }}>{t("标记频率：")}</span>
                          ...
                          return (
                            <div>
                              ...
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                                {[
                                  { label: t("🌱盲点"), range: '1–3', color: '#f87171' },
                                  { label: t("🌿成长中"), range: '4–7', color: '#fbbf24' },
                                  { label: t("🌳稳定操练"), range: '8–10', color: '#4ade80' },
                                ].map(z => (
                              ...
                              <div style={{ textAlign: 'right', marginTop: '4px', height: '18px' }}>
                                {activeOpt && (
                                  <span style={{ fontSize: '12px', color: trackColor, fontWeight: 600, transition: 'color 0.2s' }}>
                                    {activeOpt.label}　<span style={{ color: 'rgba(255,255,255,0.3)', fontWeight: 400 }}>{sliderVal}{t("分")}</span>
                                  </span>
                                )}
                              </div>
                            </div>
            ...
            return (
              <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <span style={{ color: '#c4b5fd', fontWeight: 600, fontSize: '15px' }}>{t("📋 答题汇总")}</span>
                  <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '13px' }}>{answered}/{total} {t("题")}</span>
                </div>
                ...
                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                  {FREQUENCY_OPTIONS.map(opt => (
                    <div key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px' }}>{freqCounts[opt.label]} {t("题")}</span>
                    </div>
                  ))}
                </div>
              </div>
