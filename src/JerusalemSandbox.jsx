    ...
    <div className="jeru">
      <div className="biblemap-head">
        ...
        <div className="biblemap-title">
          <h2>{i18nT('🏛 耶路撒冷数字孪生沙盘')}</h2>
          <p>{i18nT('圣城变迁与圣殿结构 · 时间轴剥离 · 受难周 FPV 步行 ·')} {engine === 'mapbox' ? 'Mapbox GL v3' : 'MapLibre GL v1（免 token）'}</p>
        </div>
      </div>
      ...
      <div className="jeru-stage">
        ...
        {status !== 'ready' && (
          <div className="jeru-overlay">
            {status === 'loading' && <div className="jeru-loading">{i18nT('🌍 正在加载')}{engine === 'mapbox' ? ' Mapbox' : ' MapLibre'} {i18nT('三维沙盘…')}</div>}
            {status === 'error' && (
              <div className="jeru-err">
                <p>⚠ {errMsg}</p>
                <p className="dim">{i18nT('该功能需要联网加载地图库与瓦片。若 Mapbox 配额/令牌不可用，会自动回退到免费的 MapLibre。')}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="jeru-controls">
          <button onClick={resetView} title={i18nT('复位到圣殿山视角')}>{i18nT('🎯 复位视角')}</button>
          {!passionActive
          ...
          {status === 'ready' && (!templeMode
            ? <button className="primary" onClick={enterTemple}>{i18nT('🏛 圣殿3D结构')}</button>
            : <>
                <button className={cutaway ? 'on' : ''} onClick={() => setCutaway(c => !c)}>{i18nT('✂ 剖视')}{cutaway ? '·开' : '·关'}</button>
                <button onClick={exitTemple}>{i18nT('🚪 离开圣殿')}</button>
              </>)}
        </div>
      </div>
      ...
      {templeMode && (
        <div className="jeru-temple-hint">
          {i18nT('🏛 所罗门第一圣殿（王上6–7，按肘比例示意复原）· 点击任一部件看经文与尺寸 · ✂ 剖视揭开殿顶察看圣所与至圣所')}
        </div>
      )}
      ...
      <div className="jeru-foot">
        {i18nT('城墙与建筑轮廓为示意性复原（schematic），用于教学呈现各时期相对范围与圣殿"平地起高楼"，非精确考古测绘。')}
