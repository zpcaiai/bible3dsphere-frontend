      ...
      {(guardians || []).map((g) => (
        <div className="cc-resource" key={g.id}>
          <div>
            ...
            <div className="meta">
              {g.phone || g.email || '—'} {i18nT('· 提醒级别：')}{LEVEL_LABEL[g.permissionLevel] || g.permissionLevel} · {g.consentEnabled ? '已授权通知' : '未授权'}
            </div>
          </div>
          <button className="cc-btn ghost" type="button" onClick={() => onDelete && onDelete(g.id)}>{i18nT('移除')}</button>
        </div>
      ))}

      <form onSubmit={submit} style={{ marginTop: 12 }}>
        <h3 style={{ fontSize: 14 }}>{i18nT('添加守护人')}</h3>
        <div className="cc-field"><label>{i18nT('姓名 / 称呼')}</label><input className="cc-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
        ...
        <div className="cc-field"><label>{i18nT('电话')}</label><input className="cc-input" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></div>
        <div className="cc-field"><label>{i18nT('邮箱（可选）')}</label><input className="cc-input" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div>
        <div className="cc-field">
          <label>{i18nT('在什么级别提醒 TA')}</label>
          <select className="cc-select" value={form.permissionLevel} onChange={(e) => setForm({ ...form, permissionLevel: e.target.value })}>
            <option value="yellow">{i18nT('我低落时')}</option>
            <option value="orange">{i18nT('我进入危机时')}</option>
            <option value="red">{i18nT('仅在紧急情况')}</option>
          </select>
        </div>
        <label style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 13, color: 'rgba(255,255,255,0.8)' }}>
          ...
          <span>{i18nT('我授权：在我进入相应级别的危机时，系统可以提醒这位守护人。我可以随时撤销。')}</span>
        </label>
        <button className="cc-btn full" type="submit" style={{ marginTop: 12 }}>{i18nT('添加')}</button>
      </form>
    </div>
  )
}
