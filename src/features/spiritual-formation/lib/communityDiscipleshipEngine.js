...
export function createAccountabilityCheckin(userId, group, data = {}) {
  ...
  return {
    checkin: {
      ...
      gratitude: data.gratitude || 'I noticed one grace this week.',
...
export function addAccountabilityResponse(userId, checkin, data = {}) {
  return {
    ...
    responseText: data.responseText || 'Thank you for sharing. I am praying and standing with you in grace.',
    createdAt: nowIso(),
    updatedAt: nowIso(),
  }
}
...
export function addMentorObservation(userId, relationship, data = {}) {
  return {
    ...
    description: data.description || 'Small evidence of grace and faithfulness was noticed.',
    evidence: data.evidence || [],
    recommendedNextStep: data.recommendedNextStep || 'Repeat one small practice and ask for help early.',
...
export function createMentorActionPlan(userId, relationship, data = {}) {
  return {
    ...
    actions: data.actions || ['practice one small habit', 'send one check-in'],
...
export function createChurchCheckin(userId, rhythm, data = {}) {
  return {
    ...
    attendedOrPracticed: data.attendedOrPracticed !== false,
    reflection: data.reflection || 'Church life practiced as embodied formation, not a checkbox.',
    encouragement: data.encouragement || '',
    obstacle: data.obstacle || '',
    nextStep: data.nextStep || 'Take one low-pressure next step.',
    createdAt: nowIso(),
  }
}
