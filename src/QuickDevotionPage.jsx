...
const QUICK_DEVOTIONS = [
  ...
  { verse: t("我靠着那加给我力量的，凡事都能做。"), ref: t("腓立比书 4:13"), question: t("此刻，有哪件事你觉得靠自己做不到？") },
  { verse: t("你们要休息，要知道我是神。"), ref: t("诗篇 46:10"), question: t("今天，你在哪件事上还没有真正停下来交托？") },
  { verse: t("因为我所受的苦楚，是神凭他的义使我受的，还是我自己犯了罪呢？"), ref: t("约伯记 10:7"), question: t("当下，神在你生命中的哪处工作让你感到困惑？") },
  { verse: t("当将你的事交托耶和华，并倚靠他，他就必成全。"), ref: t("诗篇 37:5"), question: t("今天，有什么计划是你握得最紧、最难放手的？") },
  { verse: t("你们祈求，就给你们；寻找，就寻见；叩门，就给你们开门。"), ref: t("马太福音 7:7"), question: t("你最近在祷告中真正渴望的是什么？") },
  { verse: t("神爱世人，甚至将他的独生子赐给他们。"), ref: t("约翰福音 3:16"), question: t("今天，神对你的爱让你感到坚定还是遥远？为什么？") },
]
      ...
      content: (
        <div>
          ...
          <textarea
            ...
            placeholder={t("简短地写下你真实的回应（也可以跳过）")}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,10,20,0.97)', zIndex: 300, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      {saved ? (
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 18, color: '#fff', fontWeight: 600 }}>{t("灵修完成！")}</div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginTop: 8 }}>{t("已存入今日灵修日记")}</div>
        </div>
      ) : (
        <div style={{ width: '100%', maxWidth: 420 }}>
          {/* Header */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
            <button onClick={onBack} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 13, cursor: 'pointer', padding: 0 }}>{t("✕ 退出")}</button>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>{t("⏱ 约2分钟")}</div>
          </div>
          ...
          <div style={{ marginTop: 28, display: 'flex', gap: 10 }}>
            {step > 0 && (
              <button onClick={() => setStep(s => s - 1)} style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, color: 'rgba(255,255,255,0.6)', fontSize: 14, cursor: 'pointer' }}>{t("← 上一步")}</button>
            )}
            {step < steps.length - 1 ? (
              <button onClick={() => setStep(s => s + 1)} style={{ flex: 2, padding: '12px', background: 'linear-gradient(135deg,#5856d6,#007aff)', border: 'none', borderRadius: 12, color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>{t("继续 →")}</button>
            ) : (
