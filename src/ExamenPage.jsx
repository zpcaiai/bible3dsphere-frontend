...
const FIELDS = [
  { key: 'consolation',   icon: '🌤', title: t("安慰 · 神的同在"),
    prompt: t("今天哪一刻，我感到被爱、被陪伴，或心里有平安？"), ph: t("一个画面、一句话、一件小事…") },
  { key: 'desolation',    icon: '🌫', title: t("枯涩 · 远离"),
    prompt: t("今天哪一刻，我感到焦虑、远离神，或失了平安？"), ph: t("诚实地写下，无需修饰…") },
  { key: 'gratitude',     icon: '🙏', title: t("感恩一件"),
    prompt: t("今天我要为哪一件事，向神说谢谢？"), ph: t("哪怕很小…") },
  { key: 'confession',    icon: '🕊', title: t("求恕 · 交托"),
    prompt: t("有什么我想求神赦免、或交在祂手中？"), ph: t("不是为了定罪，是为了被释放…") },
  { key: 'tomorrow_step', icon: '🌱', title: t("明日一个微顺服"),
    prompt: t("明天我可以忠心去做的一件小事是什么？"), ph: t("一个具体、微小、可完成的行动…") },
]
    ...
    <div style={{ width: '100%', height: '100%', background: '#000', color: '#fff', overflowY: 'auto', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(28,28,30,0.92)', position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(10px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <BackButton onClick={view === 'history' ? () => setView('today') : onBack} />
          <div>
            <div style={{ fontSize: 17, fontWeight: 600 }}>{t("今日省察")}</div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)' }}>{t("依纳爵式 Examen · 与神同回顾这一天")}</div>
          </div>
        </div>
        <button onClick={view === 'history' ? () => setView('today') : openHistory} style={pill}>
          {view === 'history' ? t("← 返回") : t("历史")}
        </button>
      </div>
      ...
      <div style={{ padding: '14px 16px 100px', maxWidth: 680, margin: '0 auto' }}>
          ...
          <>
            <div style={{ ...card, background: 'linear-gradient(135deg, rgba(139,92,246,0.12), rgba(90,200,250,0.08))' }}>
              <div style={{ fontSize: 13, lineHeight: 1.75, color: 'rgba(255,255,255,0.8)' }}>
                {t("安静一分钟。让这一天在神面前重演一遍——不为打分，只为看见祂在哪里，\n                也把心交还给祂。")}
              </div>
            </div>
            ...
            <div style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
                <span style={{ color: 'rgba(255,255,255,0.8)' }}>{t("今天，我感到与神有多亲近？")}</span>
            ...
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', textAlign: 'center', marginTop: 14, lineHeight: 1.6 }}>
              {t("每天一条；今天的省察会自动覆盖更新。这是温柔的回顾，不是考核。")}
            </div>
          </>
        )}
        ...
        {view === 'history' && (
          history.length === 0
            ? <div style={{ ...card, textAlign: 'center', color: 'rgba(255,255,255,0.5)' }}>{t("还没有省察记录")}</div>
            : history.map(e => (
              <div key={e.id} style={card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: 12, fontWeight: 600 }}>{e.entry_date}</span>
                  <span style={{ fontSize: 11, color: '#a78bfa' }}>{t("亲近感")} {Math.round(e.consolation_level)}</span>
                </div>
